...
func cmdRegenerateVhostToken(vhostPath string) {
	...
	s.Start("Regenerating an access token")
	cli := &http.Client{}
	form := make(url.Values)
	form.Set("path", vhostPath)
	req, _ := http.NewRequest("PUT", urlFor("/vhost/token?%s", form.Encode()), nil)
	...
	if vhost, ok := data.(map[string]interface{}); ok {
		...
		if !ok || token != "" {
			s.Ok()
			fmt.Printf("---\n")
			fmt.Printf("New access token for this vhost: %s\n", token)
			return
		}
	}
error:
	s.Fail("couldn't regenerate an access token (is server running?)", true)
}
