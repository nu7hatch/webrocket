...
const (
	KCOREADER   int = C.KCOREADER
	KCOWRITER   int = C.KCOWRITER
	KCOCREATE   int = C.KCOCREATE
	KCOTRUNCATE int = C.KCOTRUNCATE
	KCOAUTOTRAN int = C.KCOAUTOTRAN
	KCOAUTOSYNC int = C.KCOAUTOSYNC
	KCONOLOCK   int = C.KCONOLOCK
	KCOTRYLOCK  int = C.KCOTRYLOCK
	KCONOREPAIR int = C.KCONOREPAIR
	KCESUCCESS  int = C.KCESUCCESS
	KCENOIMPL   int = C.KCENOIMPL
	KCEINVALID  int = C.KCEINVALID
	KCENOREPOS  int = C.KCENOREPOS
	KCEBROKEN   int = C.KCEBROKEN
	KCENOPERM   int = C.KCENOPERM
	KCEDUPREC   int = C.KCEDUPREC
	KCENOREC    int = C.KCENOREC
	KCELOGIC    int = C.KCELOGIC
	KCESYSTEM   int = C.KCESYSTEM
	KCEMISC     int = C.KCEMISC
	KCMADD      int = C.KCMADD
	KCMSET      int = C.KCMSET
	KCMAPPEND   int = C.KCMAPPEND
	KCMREPLACE  int = C.KCMREPLACE
)
...
func (kc *KCDB) IncrDouble(key []byte, amount float64) (err error) {
	ckey := (*C.char)(unsafe.Pointer(&key[0]))
	if C.kcdbincrdouble(kc.db, ckey, C.size_t(len(key)), C.double(amount), 0) == 0 {
		err = kc.error()
	}
	return
}
